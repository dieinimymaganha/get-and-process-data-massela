...
class ProcessFiles:
    def __init__(self, uri_connection):
        ...
        self.local_files = glob.glob(
            f'{os.path.join(os.getcwd(), "get_and_process_data_massela/files/")}*.xls')
    ...
    def get_modalitys(self):
        df_modality = pd.read_sql('select id_modality as modality_id, '
                                  'description as modality from modality',
                                  con=self.connection.engine)
        self.df_modality = df_modality.set_index('modality', inplace=True)

    def process_students(self):
        df_students = pd.read_excel(os.path.join(os.getcwd(),
                                                 'get_and_process_data_massela/files/Alunos.xls'))
        df_students.drop(
            ['Telefone', 'CPF', 'RG', 'E-mail', 'Endereço', 'Data Entrada',
             'Tamanho da Camiseta', 'Status'], axis=1, inplace=True)
        df_students.rename(columns={"Nome": "name", "Sexo": "gender",
                                    "Data de Nascimento": 'birth_date'},
                           inplace=True)
        df_students.birth_date = df_students.birth_date.apply(
            lambda x: self.convert_date(x))

        self.upsert_students(df_students=df_students)

    def upsert_students(self, df_students):
        query = text(""" 
            ...
            df_students.to_records(index=False))]).replace('"', "'"))
        self.connection.engine.execute(query)

    def get_students_postgres(self):
        df_students_postgresql = pd.read_sql(
            'select id_student as student_id, '
            'name as name_student '
            'from students',
            con=self.connection.engine)
        self.df_students_postgresql = df_students_postgresql.set_index(
    ...
    def convert_type_columns_workouts(self):
        ...
        self.df_workouts.training_date = self.df_workouts.training_date.apply(
            lambda x: parse(x))
    ...
    def upsert_workouts(self, df_final):
        query = text(""" 
             ...
             """ % ','.join(
            [str(i) for i in list(df_final.to_records(index=False))]).replace(
            '"', "'"))
        self.connection.engine.execute(query)
